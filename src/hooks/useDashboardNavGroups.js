...
async function buildDefaultGroups({ user, empresa, permisosUsuario, canUseAgente }) {
  ...
  if (permisosUsuario.includes("GESTIONAR_PROVEEDORES")) {
    finanzasItems.push({ title: "Proveedores", path: "/proveedores", icon: icon(StoreIcon) });
  } else if (permisosUsuario.includes("VER_CUENTA_CORRIENTE_PROVEEDORES")) {
  ...
  if (permisosUsuario.includes("GESTIONAR_PROVEEDORES") || permisosUsuario.includes("VER_CUENTA_CORRIENTE_PROVEEDORES")) {
    finanzasItems.push({ title: "Pagos", path: "/pagos", icon: icon(PaymentsIcon) });
  }
