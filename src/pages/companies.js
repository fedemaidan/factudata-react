...
const Page = () => {
  ...
  const [newFacturaEmisor, setNewFacturaEmisor] = useState("")
  ...
  const onSubmitFactura = async () => {
    try {
      await addDoc(facturasCollectionRef, {emisor: newFacturaEmisor})
      getFacturasList();
    }
    catch (err) {
      console.error(err)
  ...
  const uploadFile = async () => {
    if (!fileUpload)
      return;
    try {
      const filesFolderRef = ref(storage, `probandoFiles/${fileUpload.name}`);
  ...
  <>
    <div>
      <input 
        placeholder='Emisor...' 
        onChange={(e) => setNewFacturaEmisor(e.target.value)}
      />
      <button onClick={onSubmitFactura}> Agregar </button>
    </div>
    {facturasList.map((factura) => (
      <div key={factura.id}>
        <p>{factura.id}</p>
        <p>{factura.emisor}</p>
        <button onClick={(e) => {deleteFactura(factura.id)} }>Delete</button>
      </div>
    ))}
    <div>
