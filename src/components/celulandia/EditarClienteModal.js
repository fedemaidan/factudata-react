...
const EditarClienteModal = ({ open, onClose, onSave, data }) => {
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await clientesService.updateCliente(data._id, {
        nombre: formData.nombre,
        descuento: formData.descuento,
        ccActivas: formData.ccActivas,
        usuario: formData.usuario,
      });

      if (result.success) {
        onSave(data._id, result.data);
        handleClose();
      } else {
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                margin="normal"
                disabled={true}
                helperText="Usuario que realizará la edición"
              />
            </Grid>
          </Grid>
        </Box>
