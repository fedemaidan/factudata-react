...
/**
 * Preview en vivo genérico para plantillas PDF generadas por IA.
 * Renderiza el componente PlantillaPDF a PDF y lo convierte a imagen (pdfjs).
 * `onImageReady` provee la imagen renderizada (usada por el corrector con visión
 * y para guardar el thumbnail). Generalización de NotaPedidoPdfPreviewInner.
 */
...
const PdfPlantillaPreviewInner = ({ PlantillaPDF, data, logoDataUrl, empresaNombre, onImageReady }) => {
  const doc = useMemo(
    () => <PlantillaPDF data={data} logoDataUrl={logoDataUrl} empresaNombre={empresaNombre} />,
    [PlantillaPDF, data, logoDataUrl, empresaNombre]
  );
  ...
  if (errorMessage) {
    console.error('[PdfPlantillaPreview] Error al renderizar PDF:', instance.error);
    return (
...
export default React.memo(PdfPlantillaPreviewInner);
